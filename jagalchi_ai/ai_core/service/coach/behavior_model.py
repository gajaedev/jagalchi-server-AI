...
class BehaviorModel:
    ...
    def assess(self, user_id: str, days: int = DEFAULT_ANALYSIS_DAYS) -> Dict[str, Any]:
        ...
        @param {str} user_id - 분석 대상 사용자 ID.
        @param {int} days - 분석 기간(일).
        @returns {Dict[str, Any]} 행동 분석 결과 딕셔너리.
        """
        # 분석 기간 설정 (최근 N일)
...
def _calculate_motivation_score(events: List[EventLog], days: int) -> float:
    ...
    @param {List[EventLog]} events - 분석 대상 이벤트 로그 목록.
    @param {int} days - 분석 기간(일).
    @returns {float} 동기 점수.
    """
    if not events:
